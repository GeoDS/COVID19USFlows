...
if f'2020-{start_month}-{start_day}'in all_time.values:
    is_valid_start = True
else:
    is_valid_start = False
    print("The start date is not a Monday. Please re-enter.")
if f'2020-{end_month}-{end_day}'in all_time.values:
    is_valid_end = True
else:
    is_valid_end = False
...
def download_file(scale, month, day, output_folder):
    ...
    try:
        if scale == "ct2ct":
            for i in range(20):
                r = requests.get(url=f"https://raw.githubusercontent.com/GeoDS/COVID19USFlows/master/weekly_flows/{scale}/{month}_{day}/weekly_{scale}_{month}_{day}_{i}.csv")
                with open(f"{output_folder}/{scale}/{month}_{day}/weekly_{scale}_{month}_{day}_{i}.csv", 'wb') as file:
                    file.write(r.content)
        else:
            r = requests.get(url=f"https://raw.githubusercontent.com/GeoDS/COVID19USFlows/master/weekly_flows/{scale}/weekly_{scale}_{month}_{day}.csv")
            with open(f"{output_folder}/{scale}/weekly_{scale}_{month}_{day}.csv", 'wb') as file:
                file.write(r.content)
        return True
...
if (is_valid_start == True) and (is_valid_end == True):
    ...
    time_df = pd.date_range(start=f'2020-{start_month}-{start_day}', end=f'2020-{end_month}-{end_day}', freq='7D')
